...
Assert-True ($regText -match 'NAV_LINK_COUNT\s*=\s*45') "NAV_LINK_COUNT = 45"
Assert-True ($regText -match '@nav-registry-count 45') "@nav-registry-count 45"
$idMatches = [regex]::Matches($regText, 'id:\s*"([^"]+)"')
Assert-True ($idMatches.Count -eq 45) "registry id count = $($idMatches.Count) (expect 45)"
$ids = @($idMatches | ForEach-Object { $_.Groups[1].Value })
Assert-True (($ids | Select-Object -Unique).Count -eq 45) "registry ids unique"
...
Assert-True ($hrefMap.Count -eq 45) "href map size 45"
...
Assert-True ($modIds.Count -eq 45) "modules flatten count = $($modIds.Count)"
Assert-True ($opsIds.Count -eq 45) "ops flatten count = $($opsIds.Count)"
Assert-True (($modIds | Select-Object -Unique).Count -eq 45) "modules no duplicate ids"
Assert-True (($opsIds | Select-Object -Unique).Count -eq 45) "ops no duplicate ids"

$modHrefs = @($modIds | ForEach-Object { $hrefMap[$_] } | Sort-Object -Unique)
$opsHrefs = @($opsIds | ForEach-Object { $hrefMap[$_] } | Sort-Object -Unique)
Assert-True ($modHrefs.Count -eq 45) "modules href unique 45"
Assert-True ($opsHrefs.Count -eq 45) "ops href unique 45"

...
export default function SerialPage() {
  const t = useTranslations("Admin.serial");
  ...
  const fetchTimeline = async (serial: SerialNumber) => {
    ...
    try {
      ...
      const { codeLabel, message } = resolveApiError(err, tErrors);
      showApiErrorToast(codeLabel, message || t("errors.loadTimelineFailed"));
    } finally {
  ...
  const handleUploadCsv = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!importForm.itemId || !importForm.locationId) {
      showApiErrorToast("", t("errors.importFieldsRequired"));
      return;
    }
    ...
    try {
      ...
      showSuccess(t("toastImportSuccess"));
      setShowImportModal(false);
      setImportForm({ itemId: "", locationId: "" });
      fetchSerials();
    } catch (err: unknown) {
      const { codeLabel, message } = resolveApiError(err, tErrors);
      showApiErrorToast(codeLabel, message || t("errors.importFailed"));
    }
  };

...
| 21 | Scale integration | ⬜ Chưa bắt đầu | Đã rà soát `rp1` và nâng đặc tả lên 96% execution-ready: khóa contract Local Agent WebSocket, scale parser profile-based, stable-weight filter, mock scale mode, manual override API/audit, permission `scale.override` và test gate bắt buộc trước nghiệm thu | — | Sẵn sàng lập plan triển khai; chưa cập nhật hoàn thành cho đến khi build/test Phase 21 pass 100%. |

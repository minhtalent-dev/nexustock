...
* Tạo module Outbound picking & packing basic
...
```text
backend/modules/outbound_picking_packing_basic/
frontend/features/outbound_picking_packing_basic/
planning/phases/phase_07_outbound_picking_packing_basic.md
```

### Permission seed đề xuất

* outbound_picking_packing_basic.read
* outbound_picking_packing_basic.create
* outbound_picking_packing_basic.update
* outbound_picking_packing_basic.approve
* outbound_picking_packing_basic.export
...
|---|---|---|
| `Shipments` | Đơn xuất | Status open,allocated,picking,packed,shipped |
| `ShipmentItems` | Dòng đơn xuất | RequestedQty, pickedQty, packedQty |
| `PickTasks` | Nhiệm vụ pick | Item, Lot, location, status |
| `PackingRecords` | Đóng gói | PackageNo, weight, status |
...
|---|---|---|
| `GET /api/shipments` | Danh sách đơn xuất | Có auth, validation, trace ID và response lỗi chuẩn. |
| `POST /api/shipments` | Tạo đơn | Có auth, validation, trace ID và response lỗi chuẩn. |
| `POST /api/shipments/{id}/generate-picks` | Sinh pick | Có auth, validation, trace ID và response lỗi chuẩn. |
| `POST /api/picks/{id}/complete` | Hoàn tất pick | Có auth, validation, trace ID và response lỗi chuẩn. |
| `POST /api/packing/{shipmentId}/complete` | Hoàn tất packing | Có auth, validation, trace ID và response lỗi chuẩn. |
...
|---|---|---|
| Shipment list | Đơn xuất | Có loading, empty, error, filter, pagination và quyền theo action. |
| Pick task screen | Nhiệm vụ lấy hàng | Có loading, empty, error, filter, pagination và quyền theo action. |
| Packing screen | Đóng gói | Có loading, empty, error, filter, pagination và quyền theo action. |
...
1. Tạo shipment
2. Sinh pick theo FIFO/FEFO
3. Operator pick
4. Packing kiểm item/qty
5. Complete ship
6. Trừ tồn
...
* Không pick Lot hold
* FIFO/FEFO mặc định
* Không ship thiếu nếu không override
* Không xuất âm

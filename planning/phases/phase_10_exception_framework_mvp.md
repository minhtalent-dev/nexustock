...
* Tạo module Exception framework MVP
...
```text
backend/modules/exception_framework_mvp/
frontend/features/exception_framework_mvp/
planning/phases/phase_10_exception_framework_mvp.md
```

### Permission seed đề xuất

* exception_framework_mvp.read
* exception_framework_mvp.create
* exception_framework_mvp.update
* exception_framework_mvp.approve
* exception_framework_mvp.export
...
|---|---|---|
| `OperationalExceptions` | Ngoại lệ vận hành | Type,severity,status,reference,reason |
...
|---|---|---|
| `POST /api/exceptions` | Tạo exception | Có auth, validation, trace ID và response lỗi chuẩn. |
| `GET /api/exceptions/open` | Danh sách mở | Có auth, validation, trace ID và response lỗi chuẩn. |
...
|---|---|---|
| Open exception dashboard | Ngoại lệ mở | Có loading, empty, error, filter, pagination và quyền theo action. |
| Exception detail | Timeline và hành động | Có loading, empty, error, filter, pagination và quyền theo action. |
| Resolve dialog | Reason và bằng chứng | Có loading, empty, error, filter, pagination và quyền theo action. |
...
* Transition hợp lệ
* Resolve cần reason
* Severity cao cần manager
* Exception không tự sửa tồn nếu chưa approve
...
* Ngoại lệ MVP quản lý tập trung, truy vết được
